...
import { syncShopifyCatalog } from '../connectors/shopify'
...
async function main() {
  ...
  if (!adminToken) missing.push('SHOPIFY_ADMIN_TOKEN')
  ...
  // TypeScript now knows these are defined since we would have exited above
  ...
  try {
    ...
    const summary = await syncShopifyCatalog(validatedConfig)
