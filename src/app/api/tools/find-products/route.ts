...
import { findProducts, validateFindProductsParams } from '../../../../../lib/mcpActions'
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (process.env.DEBUG_HTTP_TOOLS) {
      console.log('[HTTP Tools] POST /api/tools/find-products', {
        method: 'find-products',
        args: body
      })
    }

    // Validate parameters
    const params = validateFindProductsParams(body)
    ...
    if (process.env.DEBUG_HTTP_TOOLS) {
      console.error('[HTTP Tools] Error in find-products:', errorMessage)
    }
